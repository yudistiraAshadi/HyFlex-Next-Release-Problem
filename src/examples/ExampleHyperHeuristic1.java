 ...
 * applications). In the extreme case, if there are no calls to the
 * hasTimeExpired() method, then the hyper-heuristic will iterate for as long as
 * the user wants. It could obtain a much better solution than the other
 * competitors by using much more time. However, this solution will never be
 * recorded for scoring purposes because the hasTimeExpired() method was never
 * called. While we have taken these reasonable steps to negate any advantage
 * from exceeding the time limit, we admit that we may not have thought of
 * everything! Therefore we issue this warning: attempting to obtain advantage
 * by exceeding the time limit is definitely not in the spirit of the
 * competiton, and will result in disqualification.
 * <p>
...
public class ExampleHyperHeuristic1 extends HyperHeuristic
{
    ...
    public void solve( ProblemDomain problem )
    {
        ...
        // it is often a good idea to record the number of low level heuristics, as this
        // changes depending on the problem domain
        ...
        // the main loop of any hyper-heuristic, which checks if the time limit has been
        // reached
        while ( !hasTimeExpired() ) {
            ...
            // the new solution is then stored at index 1 of the solution memory while we
...
//            System.out.println("Heuristic: " + heuristic_to_apply + ", function value: " + new_obj_function_value);
            ...
            // A smaller fitness means a better solution.
            if ( delta > 0 ) {
...
//                    System.out.println("WRS Heuristic: " + heuristic_to_apply + ", function value: " + current_obj_function_value);
            ...
            // and check if the time has expired
        }
    }
